...
function App() {
  ...
  if (page === "vendor-register") return <VendorRegistration onSuccess={() => { setTab("home"); setPage("app"); }} onSwitch={() => setPage("login")} />;

  return (
    <div className="app-shell">
      <Topbar onNav={setTab} tab={tab} />
      {tab === "home" && user?.role === "admin" && <AdminDashboard />}
      {tab === "home" && user?.role === "vendor" && <VendorDashboard />}
      {tab === "home" && user?.role !== "admin" && user?.role !== "vendor" && <FarmerHome />}
